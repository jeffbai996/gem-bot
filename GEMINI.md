...
- **Text-to-Image Generation (`/gemini image`):** Dedicated slash command (`/gemini image prompt:<text> [ratio:<1:1|16:9|9:16|4:3|3:4]>`) wired via `generateImage` in `src/image-generation.ts`. Powered by Gemini 3.1 Flash Image (`gemini-3.1-flash-image`), renders with token and cost metadata footer, gated to allowlisted users, defers reply immediately, and posts generated images without false triggers in regular chat.
...
Hot reload (no restart — reloads `access.json` and persona files only):
...
3. Diagnose based on the counter values:
   - `frames_sent > 0` + `msgs_received == 0` → config issue (auth,
     model, or Live API mode)
   - `audio_chunks > 0` but no playback → Node playback / Opus framing
   - websocket close before frames → session lifecycle / permission issue
