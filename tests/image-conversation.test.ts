...
test('provider parser preserves image actions in both reply forms', () => {
  const action = JSON.stringify({ image_request: { prompt: 'An orange cat' } })
  assert.equal(parseImageRequest(parseResponse(action).reply)?.prompt, 'An orange cat')
  assert.equal(parseImageRequest(parseResponse(JSON.stringify({ reply: action })).reply)?.prompt, 'An orange cat')
})
