...
test('accepts a valid relay targeted to gemma and preserves owner identity', () => {
  const verifier = new TrustedRelayVerifier(() => config)
  assert.deepEqual(verifier.verify({
    messageId: 'message-1',
    channelId: 'channel-1',
    authorId: 'helper-bot',
    content: marker('gemma', 'You chose option 1: proceed'),
  }), {
...
test('rejects broadcasts so only explicitly targeted relays wake this bot', () => {
  const verifier = new TrustedRelayVerifier(() => config)
  assert.equal(verifier.verify({
    messageId: 'message-3',
    channelId: 'channel-1',
...
test('rejects bad signatures and ordinary bot chatter', () => {
  ...
  assert.equal(verifier.verify({
    ...
    channelId: 'channel-1',
    authorId: 'helper-bot',
    content: `⟦vc-relay:gemma:${'0'.repeat(64)}⟧ forged`,
  }), null)
  assert.equal(verifier.verify({
    messageId: 'message-5',
    channelId: 'channel-1',
...
test('rejects a duplicate Discord delivery of an already consumed relay', () => {
  ...
  const input = {
    ...
    channelId: 'channel-1',
    authorId: 'helper-bot',
    content: marker('gemma', 'proceed once'),
  }
...
test('Discord ingress verifies bot messages instead of dropping them wholesale', async () => {
  ...
  assert.match(inbound, /await handleInboundMessage\(message, acceptedRelay\)/)
  assert.doesNotMatch(
    inbound,
    /if \(acceptedRelay\) void message\.delete\(\)/,
    'the helper owns and reaps its relay posts; this bot cannot delete another bot\'s message',
  )
})
