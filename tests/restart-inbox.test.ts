...
const tmp = path.join(os.tmpdir(), `gem-restart-inbox-${process.pid}`)
const file = path.join(tmp, 'restart-inbox.json')

afterEach(() => fs.rmSync(tmp, { recursive: true, force: true }))

test('deduplicates and replays messages from the restart cutover', async () => {
  ...
  const count = await new RestartInbox(file).replay(async (channelId, messageId) => {
...
test('retains failed replay entries for the next boot', async () => {
  const inbox = new RestartInbox(file)
  inbox.defer('channel-a', 'message-1')
  inbox.defer('channel-b', 'message-2')
