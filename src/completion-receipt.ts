import type { ToolCall } from './gemini.ts'
...
function testSummary(calls: ToolCall[]): string | null {
  const tests = calls.filter(call => !call.failed && TEST_COMMAND_RE.test(commandOf(call)))
...
export function buildCompletionReceipt(toolCalls: ToolCall[], writtenFiles: string[] = []): string | null {
  const files = new Set(writtenFiles.filter(Boolean))
  for (const call of toolCalls) {
    if (call.failed) continue
    const file = String(call.args?.file_path ?? call.args?.path ?? '').trim()
    if (file && /(?:edit|write|patch|file)/i.test(call.name)) files.add(file)
  }

  const shellCalls = toolCalls.filter(call => !call.failed && /(?:shell|command|exec)/i.test(call.name))
  const tests = testSummary(shellCalls)
  const commit = shellCalls
    .map(call => call.resultPreview?.match(/\[[^\]]+\s+([0-9a-f]{7,40})\]/i)?.[1])
    .find(Boolean)
  const deployed = shellCalls.some(call => DEPLOY_COMMAND_RE.test(commandOf(call)))
  ...
  return `-# ▸ work receipt · ||${facts.join(' · ')}||`
}
