...
const CUSTOM = /^(<a?:[A-Za-z0-9_]+:\d+>)\s+(.+)$/u
const EMOJI = /^(\p{Extended_Pictographic}(?:\uFE0F|\u200D\p{Extended_Pictographic})*|[\p{Regional_Indicator}]{2})\s+(.+)$/u
export function parsePollAnswers(spec: string): PollData['answers'] {
  ...
  return parts.map(part => { const m = part.match(CUSTOM) ?? part.match(EMOJI); const text = (m?.[2] ?? part).trim(); if (!text || text.length > 55) throw new Error('each poll option must be 1–55 characters'); return m ? { text, emoji: m[1] } : { text } })
}
export function buildPoll(question: string, options: string, duration = 24, allowMultiselect = false): PollData {
  const q = question.trim(); if (!q || q.length > 300) throw new Error('poll question must be 1–300 characters')
