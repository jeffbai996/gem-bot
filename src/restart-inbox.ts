...
/** Durable handoff for messages arriving during the final restart cutover. */
export class RestartInbox {
  ...
  async replay(handle: (channelId: string, messageId: string) => Promise<void>): Promise<number> {
    let replayed = 0
    for (const item of [...this.items]) {
      try {
        ...
        // Transient Discord failures stay queued for the next boot.
      }
    }
    return replayed
  }
