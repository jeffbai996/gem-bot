...
export function resolveReplyContext(message: MessageLike): Promise<ReplyContext | null> {
  if ([CHANNEL_PINNED_MESSAGE, THREAD_CREATED, THREAD_STARTER_MESSAGE].includes(message.type ?? -1)) return Promise.resolve(null)
...
export function resolveThreadContext(message: MessageLike): Promise<ThreadContext | null> {
  if (message.type !== THREAD_CREATED && message.type !== THREAD_STARTER_MESSAGE) return Promise.resolve(null)
  ...
  const pending = (async (): Promise<ThreadContext | null> => {
    ...
    if (message.type === THREAD_CREATED) {
      ...
      return { kind: 'created', threadId, threadName, parentChannelId, forumTagIds, starterContent: message.content ?? '', source: null }
    }
    const source = message.reference?.messageId ? await fetchReferencedMessage(message) : null
    return {
      kind: 'starter', threadId: message.channelId ?? message.id ?? 'unknown',
...
export function resolvePinContext(message: MessageLike): Promise<PinContext | null> {
  if (message.type !== CHANNEL_PINNED_MESSAGE || !message.reference?.messageId) return Promise.resolve(null)
...
export function formatReplyContext(context: ReplyContext | null): string {
  ...
  return '[Discord reply context — the user explicitly replied to this earlier message]\n'
    + JSON.stringify({
      ...
      attachments: context.attachments.map(({ name, contentType, size }) => ({
...
export function formatPinContext(context: PinContext | null): string {
  if (!context) return ''
  if (!context.message) return `[Discord pin event — the user pinned message ${context.messageId}, but its content is unavailable]`
...
export function formatThreadContext(context: ThreadContext | null): string {
  if (!context) return ''
  const thread = { thread_id: context.threadId, thread_name: context.threadName, parent_channel_id: context.parentChannelId, forum_tag_ids: context.forumTagIds }
  ...
  return '[Discord thread starter — this thread was created from an earlier message]\n' + JSON.stringify({
    ...
    source_message: context.source ? {
      message_id: context.source.messageId, author: context.source.authorName,
      author_id: context.source.authorId, content: context.source.content,
      attachments: context.source.attachments.map(({ name, contentType, size }) => ({ name, content_type: contentType, size })),
    } : null,
